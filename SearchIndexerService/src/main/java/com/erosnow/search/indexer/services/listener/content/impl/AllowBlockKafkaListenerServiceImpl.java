...
@Service(Listener.ALLOW_BLOCK_KAFKA_LISTENER)
@Scope("prototype")
public class AllowBlockKafkaListenerServiceImpl extends AbstractKafkaListenerServiceImpl {

	private static final Logger LOG = LoggerFactory.getLogger(AllowBlockKafkaListenerServiceImpl.class);

	protected Set<String> ids = new HashSet<String>();

	public AllowBlockKafkaListenerServiceImpl() {
		super(Listener.ALLOW_BLOCK_KAFKA_LISTENER);
	}
	...
	public synchronized void index() {
		...
		try {
			...
			getContentIndexerService().indexAllowedOrBlocked(contentIds);
		} catch (Exception e) {
